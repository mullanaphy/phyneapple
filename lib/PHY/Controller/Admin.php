<?php
    ...
    class Admin extends AController
    {
        ...
        public function index_get()
        {
            ...
            $collection->raw(true)->where()->field('key')->in(['address', 'phone', 'email']);
            foreach ($collection as $row) {
                $content->setVariable($row['key'], $row['value']);
            }
            $content->setVariable('user', $app->getUser());

            $collection = $manager->getCollection('User');
            $content->setVariable('users', $collection);

            $collection = $manager->getCollection('Blog');
            $collection->limit(3);
            $content->setVariable('news', $collection);
        }
        ...
        public function closing_get()
        {
            ...
            if ($id !== false) {
                if ($id) {
                    ...
                    $item = new Closing($request->get('closing', []));
                }
                $content->setTemplate('admin/closing/item.phtml');
                $content->setVariable('item', $item);
                $breadcrumb = $layout->block('breadcrumb');
                $breadcrumb->setVariable('item', $item);
            } else {
                ...
                $collection = $manager->getCollection('Closing');
                $collection->limit((($pageId * $limit) - $limit), $limit);
                $collection->order()->by('date', 'desc');

                $content->setTemplate('admin/closing/collection.phtml');
                $content->setVariable('collection', $collection);
                $content->setChild('pagination', [
                    'viewClass' => 'pagination',
                    'pageId' => $pageId,
                    ...
                    'total' => $collection->count(),
                    'url' => [
                        $this->url('admin/closing'),
                        'limit' => $limit
                    ]
                ]);
            }
            if ($message = $app->get('session/admin/closing/message')) {
                $app->delete('session/admin/closing/message');
                $message['template'] = 'generic/message.phtml';
                $content->setChild('message', $message);
            }
        }
